...
//    writeToTxt()
    ...
    var vertexRDD: RDD[(Long,Boolean)] = sc.parallelize(searchMap.map((m:(Long,(Long,Long,Long)))=>(m._1,true)).toSeq)
...
//    val subgraph = sssp.subgraph(vpred = (vid,tag)=>tag)
  ...
  /**
    ...
    * @return
    */
  def initVertexAndEdge(triple:utils.Triple): Unit={
    ...
    for (row <- resultrows) {
      ...
      //如果对应的定点和边都添加过则不再重复递归，防止死循环
      if(vertexMap.contains(vertexTo) && edgeMap.contains((triple.rsubject,triple.rpredicate,triple.robject,triple._type.toInt))){
        return
      }
